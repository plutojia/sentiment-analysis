...
lstm_layer_forwards=LSTM(HIDDEN_LAYER_SIZE, dropout=0.3, recurrent_dropout=0.3,return_sequences=True)(embedded_sequences)
lstm_layer_backwards=LSTM(HIDDEN_LAYER_SIZE, dropout=0.3, recurrent_dropout=0.3,return_sequences=True,go_backwards=True)(embedded_sequences)
print(lstm_layer_backwards.shape)
concate_layer=Concatenate(axis=2)([lstm_layer_forwards,lstm_layer_backwards])
print(concate_layer.shape)

flattened=Flatten()(concate_layer)
dropouted_2=Dropout(0.4)(flattened)
dense_1=Dense(256, activation='relu')(dropouted_2)
dropouted_3=Dropout(0.3)(dense_1)
preds = Dense(2, activation='softmax')(dropouted_3)
...
model.save(os.path.join(model_dir,'biLSTM_model.h5'),overwrite=True)
